...
export type AppUser = {
  ...
  tenantId: string; tenantSlug: string; tenantRole: "member" | "tenant_admin";
...
export function ensureStore() {
  ...
  setup = (async () => {
    await database.batch([
      ...
      database.prepare("CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, email TEXT NOT NULL COLLATE NOCASE UNIQUE, password_hash TEXT NOT NULL, password_salt TEXT NOT NULL, role TEXT NOT NULL DEFAULT 'user', first_name TEXT, last_name TEXT, phone TEXT, company_role TEXT, created_at TEXT NOT NULL, last_login_at TEXT)"),
    ...
    const forceAdminPasswordReset = String(process.env.ADMIN_FORCE_PASSWORD_RESET || "").trim().toLowerCase() === "true";
    if (adminEmail && adminPassword) {
      ...
      if (!existing) {
        ...
        await database.prepare("INSERT INTO users (id, email, password_hash, password_salt, role, first_name, created_at) VALUES (?, ?, ?, ?, 'admin', ?, ?)").bind(crypto.randomUUID(), adminEmail, password.hash, password.salt, "Администратор", now).run();
      } else if (forceAdminPasswordReset) {
        const password = await passwordHash(adminPassword);
        await database.prepare("UPDATE users SET role = 'admin', password_hash = ?, password_salt = ? WHERE id = ?").bind(password.hash, password.salt, existing.id).run();
      } else await database.prepare("UPDATE users SET role = 'admin' WHERE id = ?").bind(existing.id).run();
    }

    await database.batch([
      database.prepare("INSERT OR IGNORE INTO tenant_memberships (tenant_id, user_id, role, created_at) SELECT ?, id, CASE WHEN role = 'admin' THEN 'tenant_admin' ELSE 'member' END, ? FROM users").bind(DEFAULT_TENANT_ID, now),
...
const userRecord = (row: AppUser | null) => row ? {
  ...row,
  role: row.role === "admin" ? "admin" as const : "user" as const,
  tenantRole: row.tenantRole === "tenant_admin" ? "tenant_admin" as const : "member" as const,
...
export async function currentUser(request: Request): Promise<AppUser | null> {
  ...
  const row = await d1().prepare("SELECT users.id, users.email, users.role, users.first_name AS firstName, users.last_name AS lastName, users.phone, users.company_role AS companyRole, tenants.id AS tenantId, tenants.slug AS tenantSlug, tenant_memberships.role AS tenantRole FROM sessions JOIN users ON users.id = sessions.user_id JOIN tenants ON tenants.id = sessions.tenant_id JOIN tenant_memberships ON tenant_memberships.tenant_id = sessions.tenant_id AND tenant_memberships.user_id = users.id WHERE sessions.token_hash = ? AND sessions.tenant_id = ? AND sessions.expires_at > ?").bind(await sha256(token), tenant.id, new Date().toISOString()).first<AppUser>();
...
export async function register(request: Request, email: string, password: string, profile: RegistrationProfile) {
  ...
  const user: AppUser = { id: crypto.randomUUID(), email: normalized, role: normalized === configuredAdmin ? "admin" : "user", tenantId: tenant.id, tenantSlug: tenant.slug, tenantRole: normalized === configuredAdmin ? "tenant_admin" : "member", firstName, lastName, phone, companyRole };
  const now = new Date().toISOString();
  await d1().batch([
    d1().prepare("INSERT INTO users (id, email, password_hash, password_salt, role, first_name, last_name, phone, company_role, created_at, last_login_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(user.id, user.email, secured.hash, secured.salt, user.role, user.firstName, user.lastName, user.phone, user.companyRole, now, now),
...
export async function login(request: Request, email: string, password: string) {
  const tenant = await tenantForRequest(request);
  const user = await d1().prepare("SELECT users.id, users.email, users.role, users.first_name AS firstName, users.last_name AS lastName, users.phone, users.company_role AS companyRole, users.password_hash, users.password_salt, tenants.id AS tenantId, tenants.slug AS tenantSlug, tenant_memberships.role AS tenantRole FROM users JOIN tenant_memberships ON tenant_memberships.user_id = users.id AND tenant_memberships.tenant_id = ? JOIN tenants ON tenants.id = tenant_memberships.tenant_id WHERE users.email = ?").bind(tenant.id, email.trim().toLowerCase()).first<AppUser & { password_hash: string; password_salt: string }>();
...
export async function changePassword(user: AppUser, currentPassword: string, newPassword: string) {
  ...
  const stored = await d1().prepare("SELECT password_hash, password_salt FROM users WHERE id = ?").bind(user.id).first<{ password_hash: string; password_salt: string }>();
  if (!stored) throw new Error("Пользователь не найден.");
  const current = await passwordHash(currentPassword, stored.password_salt);
  ...
  await d1().prepare("UPDATE users SET password_hash = ?, password_salt = ? WHERE id = ?").bind(next.hash, next.salt, user.id).run();
}
...
export async function requireAdmin(request: Request) {
  ...
  if (!user || (user.role !== "admin" && user.tenantRole !== "tenant_admin")) return null;
  return user;
}
