...
if (typeof window !== "undefined" && !(window as Window & { __roomPlanControls?: boolean }).__roomPlanControls) {
  ...
  const keepMenuHorizontal = (item: HTMLElement) => {
    const rotation = item.style.transform.match(/rotate\(([-\d.]+)deg\)/)?.[1] || "0";
    item
      .querySelector<HTMLElement>(".plan-context-menu")
      ?.style.setProperty("--no-rotation", `${rotation}deg`);
  };

  document.addEventListener("contextmenu", (event) => {
    const item = (event.target as HTMLElement | null)?.closest<HTMLElement>(".planogram-furniture");
    if (item) later(() => { keepMenuHorizontal(item); attachCatalog(item); attachDelete(); });
  });
}
