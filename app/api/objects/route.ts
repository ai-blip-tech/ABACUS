...
export async function POST(request: Request) {
  ...
  try {
    ...
    await ensureStore();
    await env.DB.prepare("INSERT OR REPLACE INTO ai_jobs (id, tenant_id, user_id, kind, created_at) VALUES (?, ?, ?, 'objects', ?)").bind(result.id, user.tenantId, user.id, new Date().toISOString()).run();
...
export async function GET(request: Request) {
  ...
  await ensureStore();
  const ownedJob = await env.DB.prepare("SELECT id FROM ai_jobs WHERE id = ? AND tenant_id = ? AND user_id = ? AND kind = 'objects'").bind(id, user.tenantId, user.id).first();
  ...
  try {
    ...
    await env.DB.prepare("DELETE FROM ai_jobs WHERE id = ? AND tenant_id = ? AND user_id = ?").bind(id, user.tenantId, user.id).run();
