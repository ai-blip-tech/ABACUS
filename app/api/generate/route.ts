...
async function generateResponse(request: Request) {
  ...
  if (!apiKey) return Response.json({ error: "Сервис генерации пока не настроен." }, { status: 503 });
  ...
  try {
    if (body.planRender?.planImage) {
      ...
      response = await fetch("https://api.openai.com/v1/images/generations", {
        ...
        body: JSON.stringify({ model: IMAGE_MODEL, prompt, size: outputSize, quality: "medium", output_format: "webp" }),
      });
    }
  } catch (error) {
