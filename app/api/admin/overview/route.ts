...
export async function GET(request: Request) {
  ...
  const users = await database.prepare(`
    SELECT users.id, users.email, users.role, users.created_at, users.last_login_at,
      ...
      ((COALESCE(SUM(generations.input_tokens), 0) * ${INPUT_USD_PER_MILLION} + COALESCE(SUM(generations.output_tokens), 0) * ${OUTPUT_USD_PER_MILLION}) / 1000000.0) * ${BRUTTO_MULTIPLIER} AS cost_brutto_usd
  ...
  const generations = await database.prepare(`
    SELECT generations.id, generations.operation, generations.prompt, generations.bytes,
      ...
      ((COALESCE(generations.input_tokens, 0) * ${INPUT_USD_PER_MILLION} + COALESCE(generations.output_tokens, 0) * ${OUTPUT_USD_PER_MILLION}) / 1000000.0) * ${BRUTTO_MULTIPLIER} AS cost_brutto_usd
