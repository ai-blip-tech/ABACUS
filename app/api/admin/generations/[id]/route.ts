...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const generation = await env.DB.prepare("SELECT output_key, content_type FROM generations WHERE id = ? AND tenant_id = ?").bind(id, admin.tenantId).first<{ output_key: string; content_type: string }>();
